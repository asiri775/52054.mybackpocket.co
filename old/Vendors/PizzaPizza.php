...
class PizzaPizza
{
    ...
    private function setVendor()
    {
        ...
        if (Str::contains(Str::lower($this->plainText), Str::lower("Placed on:"))) { //Plain email body

            if (Str::contains(Str::lower($this->plainText), Str::lower('Placed on:'))) {

                $date_text = preg_grep('/Placed on:/', $this->textArray);
                $date_text = str_replace(['Placed on:', 'at'], '', implode($date_text));
    ...
    private function setExtraInfo()
    {
        ...
        if (Str::contains(Str::lower($this->plainText), Str::lower('Your Order Confirmation'))) {
                ...
                $ex_info[] = [
                    ...
                    'value' => floatval(
                        str_replace(
                            '$',
                            '',
                            $this->textArray[$discount_label_index + 1]
                        )
                    ),
                    'key' => 'discount',
                    'type' => 'amount'
                ...
                $ex_info[] = [
                    ...
                    'value' => floatval(
                        str_replace(
                            '$',
                            '',
                            $this->textArray[$dc_label_index + 1]
                        )
                    ),
                    'key' => 'delivery_charge',
                    'type' => 'amount'
                ...
                $ex_info[] = [
                    ...
                    'value' => floatval(
                        str_replace(
                            '$',
                            '',
                            $this->textArray[$pst_label_index + 1]
                        )
                    ),
                    'key' => 'pst',
                    'type' => 'amount'
            ...
            if (Str::contains(Str::lower($redeemed_label), Str::lower('Redeemed'))) {
                $ex_info[] = [
                    'label' => str_replace(':', '', $redeemed_label),
                    'value' => floatval(
                        str_replace(
                            '$',
                            '',
                            $this->textArray[$total_label_index + 3]
                        )
                    ),
    ...
    private function setTransaction()
    {
        if (Str::contains(Str::lower($this->plainText), Str::lower('Your Order Confirmation'))) {
            ...
            $this->sub_total = floatval(str_replace('$', '', $trans_start[$sub_total_index + 1]));
            $this->tax_amount =  floatval(str_replace('$', '', $trans_start[$tax_label_index + 1]));
            $this->total =  floatval(str_replace('$', '', $trans_start[$total_label_index + 1]));
        } else {
            $total_label_index = array_search('Total:', $this->textArray);
