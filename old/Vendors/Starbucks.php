...
class Starbucks
{
    ...
    private function setVendor()
    {
        ...
        if (Str::contains(Str::lower($this->plainText), Str::lower("Received:"))) {
            //TODO: check if there are more than forwarded messages
            $date_text = Str::between($this->htmlBody, "Received:", "Billing Address:");
            ...
            //If string contains fails then use the default email address
            if (Str::contains($this->htmlBody, 'starbucks')) {
    ...
    private function setProducts()
    {
        ...
        foreach ($prods_array as $key => $value) {
            if (
                ...
                $prods[$key]['price'] = floatval(str_replace(['$CAD', 'CAD', '$', 'CA', ','], '', $prods_array[$key][4]));
            }
        }
    ...
    private function setTransaction()
    {
        ...
        $this->sub_total = floatval(str_replace(['$CAD', '$', 'CAD', ','], '', $this->textArray[$sub_total_index + 1]));
        $this->tax_amount = floatval(str_replace(['$CAD', '$', 'CAD', ','], '', $this->textArray[$tax_label_index + 1]));
        $this->total = floatval(str_replace(['$CAD', '$', 'CAD', ','], '', $this->textArray[$total_label_index + 1]));
        $this->payment_method = "";
    }
