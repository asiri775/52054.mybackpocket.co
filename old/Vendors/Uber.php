...
class Uber
{
    ...
    {
        if (Str::contains(Str::lower($this->plainText), Str::lower("You ordered from"))) {
            ...
            return true;
        } elseif (Str::contains(Str::lower($this->plainText), Str::lower("You rode with"))) {
    ...
    private function setVendor()
    {
        ...
        if (Str::contains(Str::lower($this->plainText), Str::lower("Total CA"))) {
            //TODO: check if there are more than forwarded messages


            $date_text = preg_grep('/Total CA/', $this->textArray);
            $date_text = $this->textArray[key($date_text) + 1];
            ...
            if (Str::contains($this->htmlBody, 'uber.canada')) {
                $this->vendor_email = "uber.canada@uber.com";
            }
        }

        if (Str::contains($this->plainText, "Terms")) {
            $address_index = array_search('Terms', $this->textArray);
            $this->vendor_address = $this->textArray[$address_index + 1];

            $this->vendor_street_name = Str::beforeLast($this->vendor_address, ' ');
            $this->vendor_city = Str::afterLast($this->vendor_address, ' ');
        }
    }
        ...
        if ($this->vendor_name == "Uber Eats") {
            ...
            foreach ($prods_array as $key => $value) {
                ...
                if (Str::contains($value, 'CA$')) {
                    $price = str_replace(['$', 'CA', 'CAD'], '', $value);
                    $price =  filter_var($price, FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);
                    $prods[] = [
                        'name' => $prods_array[$key - 1],
                        'quantity' => is_numeric($prods_array[$key - 2]) ? intval($prods_array[$key - 2]) : 0,
            ...
            $trip_fare_index = preg_grep("/Trip Fare/", $prods_array);
            $prods[] = [
                'name' => "Trip Fare",
                'price' => floatval(str_replace(
                    ['$', 'CA', 'CAD'],
                    '',
                    ($prods_array[key($trip_fare_index) + 1])
                ))
            ];
        }
            ...
            if ($extra_tax_label_index) {
                $ex_info[] = [
                    ...
                    'value' => floatval(str_replace(
                        ['$', 'CA', 'CAD'],
                        '',
                        $this->textArray[$extra_tax_label_index + 1]
                    )),
                    'key' => 'extra_tax',
                    'type' => 'amount'
                ];
            ...
            if ($service_fee_label_index) {
                $ex_info[] = [
                    ...
                    'value' => floatval(str_replace(
                        ['$', 'CA', 'CAD'],
                        '',
                        $this->textArray[$service_fee_label_index + 1]
                    )),
                    'key' => 'service_fee',
                    'type' => 'amount'
                ];
        ...
        if ($special_offer_label_index) {
            $ex_info[] = [
                ...
                'value' => floatval(str_replace(
                    ['$', 'CA', 'CAD'],
                    '',
                    $this->textArray[$special_offer_label_index + 1]
                )),
                'key' => 'special_offer',
                'type' => 'amount'
            ];
        ...
        if ($promotion_label_index) {
            $ex_info[] = [
                ...
                'value' => floatval(str_replace(
                    ['$', 'CA', 'CAD'],
                    '',
                    $this->textArray[$promotion_label_index + 1]
                )),
                'key' => 'promotion',
                'type' => 'amount'
            ];
    ...
    private function setTransaction()
    {
        ...
        $this->sub_total = floatval(str_replace(['$', 'CA', 'CAD'], '', $this->textArray[$sub_total_index + 1]));
        $this->tax_amount = floatval(str_replace(['$', 'CA', 'CAD'], '', $this->textArray[$this->tax_label_index + 1]));

        $this->total = floatval(str_replace(['$', 'CA', 'CAD'], '', $this->total));
