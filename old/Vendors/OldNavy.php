...
class OldNavy
{
    ...
    private function setVendor()
    {
        ...
        $this->vendor_email = Str::between($this->plainText, 'Old Navy Canada <', '>To:');
        $this->vendor_email = explode('--tagend', $this->vendor_email);
        $this->vendor_email = $this->vendor_email[0];

        $this->vendor_address = Str::between($this->htmlBody, "OLD NAVY -", "Tel");
    ...
    private function setProducts()
    {
        ...
        $productsHtml = Str::between($this->htmlBody, 'EXCHANGE', 'Total Discount');
    ...
    private function setTransaction()
    {
        ...
        $this->sub_total = explode('GST', $this->sub_total);
        $this->sub_total = $this->sub_total[0];
        $this->sub_total = ltrim($this->sub_total, '-');

        $this->total = Str::between($amounts, 'TotalTender', 'ChangeDue');
        $this->total = $this->removeExtraWords($this->total);
        $this->total = ltrim($this->total, '-');

        $this->tax_amount = Str::between($amounts, 'TotalTax', 'Total');
        $this->tax_amount = $this->removeExtraWords($this->tax_amount);
        $this->tax_amount = ltrim($this->tax_amount, '-');
        ...
        $this->payment_method = trim($this->payment_method[0]);
        $this->payment_method = Str::before($this->payment_method, 'Refunded');
        ...
        $this->discount = $this->removeExtraWords($this->discount);
        $this->discount = Str::after($this->discount, 'Total Discount');
        $this->discount = preg_replace('/[\x00-\x1F\x7F-\xFF]/', '', $this->discount);
        $this->discount = Str::after($this->discount, 'TotalDiscount');
