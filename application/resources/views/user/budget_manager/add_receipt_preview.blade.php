    ...
    <div class="container-fluid container-fixed-lg">
        ...
        <div class="card card-default">
            ...
            <div class="card-body p-t-20">
                ...
                <form id="store-budget" action="{{ route('user.budget.add.receipt.store') }}" method="POST">
                        ...
                        <table class="table table-hover table-condensed table-responsive-block table-responsive"
                            ...
                            <tbody>
                                @foreach ($transactions as $transaction)
                                    <tr>
                                        <td><input type="checkbox" name="transId[]" value="{{ $transaction->id }}" style="text-align: center;"></td>
                                        <td class="text-center" style="text-align: center;">{{ $transaction->transaction_no }}</td>
                                        <td style="text-align: center;"> {{ $transaction->vendor->name }}</td>
                                        <td style="text-align: center;">{{ $transaction->order_no }}</td>
                                        <td style="text-align: center;">{{ $transaction->payment_method }}</td>
                                        <td style="text-align: center;">${{ number_format((float)$transaction->total, 2, '.', '') }}</td>
                                        <td style="text-align: center;"><a class="btn btn-danger"
                                                href="{{ route('user.budget.add.receipt.delete' , $transaction->id) }}">Delete</a>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
                                    ...
                                    <div class="row">
                                        <div class="col-xs-12">
                                            <label style="font-size: 14px;"><b>Summary :
                                                    {{ $transactions->count() }}&nbsp;Items&nbsp;|&nbsp;${{ number_format((float)$grandTotal, 2, '.', '') }}</b></label>
                                        </div>
                                    </div>
                                </div>
                            </div>
