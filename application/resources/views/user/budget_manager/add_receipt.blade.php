        ...
        <div class="card card-default  p-l-20  p-r-20">
            ...
            <div class="p-b-10">
                ...
                <div class="pull-right">
                    <a href="{{ route('user.budget.add.receipt.preview') }}" style="vertical-align: middle;"
...
@section('page-js')
    ...
    <script>
        ...
        $(document).ready(function(e) {
            var table = $('#budgetAddReceiptsTable');
            ...
            var trans_datatable = table.DataTable({
                ...
                "ajax": {
                    "url": "{{ route('user.budget.add.receipt.datatable') }}",
                    "method": "POST",
                    'data': function(data) {
           ...
           $('#to').datepicker({
            ...
            $('.buttons-select-all').on('click', function() {
                ...
                var urlpost = "{{ route('user.budget.add.receipt.bulkSession') }}";
            ...
            $('.buttons-select-none').on('click', function() {
                ...
                var urlpost = "{{ route('user.budget.add.receipt.clear.session') }}";
            ...
            $('#budgetAddReceiptsTable').on('click', 'input', function() {
                ...
                if ($(this).is(':checked')) {
                    array.push(id);
                    var urlpost = "{{ route('user.budget.add.receipt.add.session') }}";
                    $.ajax({
                        type: 'POST',
                        url: urlpost,
                        data: {
                    ...
                    var urlpost = "{{ route('user.budget.add.receipt.remove.session') }}";
                    $.ajax({
                        type: 'POST',
                        url: urlpost,
                        data: {
