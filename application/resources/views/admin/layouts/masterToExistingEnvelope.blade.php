...
<script type="text/javascript">
    $('.cancel-confirmation').on('click', function () {
        return confirm('Do you really want to cancel deposit?');
    });
    ...
    function filterColumn(i) {
        $('#existingEnvelopeTable').DataTable().column(i).search(
            $('#col' + i + '_filter').val(),
        ).draw();
    }
    ...
    $('.new-picker').on('apply.daterangepicker', function (ev, picker) {
        $(this).val(picker.startDate.format('MM/DD/YYYY'));
    });

    $('.new-picker').on('cancel.daterangepicker', function (ev, picker) {
        $(this).val('');
    });
    $(document).on('change', '[type="radio"]', function() {
        ...
        var id = $(this).val();
        if ($(this).is(':checked')) {
            var urlpost = "{{route('existingEnvelope.addSession')}}";
            $.ajax({
                type: 'POST',
                url: urlpost,
                data:{id:id},
            ...
            var urlpost = "{{route('existingEnvelope.removeSession')}}";
            $.ajax({
                type: 'POST',
                url: urlpost,
                data:{id:id},
    ...
    {{--    window.location.replace("{{route('select-existing-deposit')}}");--}}
    // });
</script>
