            ...
            @foreach($transaction->purchase as $purchase)
                <tr>
                    ...
                    <td style="border-right: 1px solid rgba(230, 230, 230, 0.7);">
                        <div
                            style="display: flex; justify-content: center; align-items: center;">{{ $purchase->quantity }}</div>
                    </td>
                    <td style="border-right: 1px solid rgba(230, 230, 230, 0.7);">
                        <div
                            style="display: flex; justify-content: flex-end; align-items: center;">{{ $purchase->price }}</div>
                    </td>
                </tr>
            @endforeach
            <tr>
                <td style="border-left: 1px solid rgba(230, 230, 230, 0.7); border-right: 1px solid rgba(230, 230, 230, 0.7);">
                    ...
                    <p class="text-uppercase mt-4" style="font-size: 1.1rem;"><span
                            class="font-weight-bold">Method: </span>{{ $transaction->payment_method }}</p>
                    <p class="text-uppercase" style="font-size: 1.1rem;"><span
                            class="font-weight-bold">Reference: </span>{{ $transaction->payment_ref }}</p>
                </td>
                <td colspan="2" style="border-right: 1px solid rgba(230, 230, 230, 0.7);">
                    <div>
                        <h5 class="text-uppercase text-right"><strong>Subtotal</strong>: ${{ $transaction->sub_total }}
                        </h5>
                        @foreach($extra_info as $info)
                            <h5 class="text-uppercase text-right"><strong>{{ $info['label'] }}</strong>:
                                ${{ $info['value'] }}</h5>
                        @endforeach
                        @if($transaction->tax_amount)
                            <h5 class="text-uppercase text-right"><strong>Taxes</strong>:
                                ${{ $transaction->tax_amount  }}</h5>
                        @endif
                        <hr>
