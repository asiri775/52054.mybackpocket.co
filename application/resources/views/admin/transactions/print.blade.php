    ...
    <div class="transaction-print container">
        <div class="row">
            <div class="col-md-4 text-center">
                @if (isset($transaction->vendor))
                <img width="20%" src="{{ asset('admin/assets/img/vendor-logos/'. $transaction->vendor->logo_vendor) }}" alt="Logo">
        ...
        <div class="row">
            <div class="col-md-4 offset-md-8">
                <p class="text-right total-top">TOTAL: $ {{ \App\Helpers\Helper::formatNumber($transaction->total) }}</p>
            </div>
        </div>
        <div class="row">
            ...
            <tbody>
                @foreach($transaction->purchase as $purchase)
                    <tr>
                        ...
                        <td style="border-right: 1px solid rgba(230, 230, 230, 0.7);">
                            <div style="display: flex; justify-content: flex-end; align-items: center;"> $ {{ \App\Helpers\Helper::formatNumber($purchase->price) }}</div>
                        </td>
                    </tr>
                @endforeach
                <tr>
                    <td style="border-left: 1px solid rgba(230, 230, 230, 0.7); border-right: 1px solid rgba(230, 230, 230, 0.7);">
                        ...
                        <p class="text-uppercase" style="font-size: 1.1rem;"><span class="font-weight-bold">Reference: </span>{{ ($transaction->payment_ref)?$transaction->payment_ref:'N/A' }}</p>
                    </td>
                    <td colspan="2" style="border-right: 1px solid rgba(230, 230, 230, 0.7);">
                        <div>
                            <h5 class="text-uppercase text-right"><strong>Subtotal</strong>: $ {{ \App\Helpers\Helper::formatNumber($transaction->sub_total)}}</h5>
                            ...
                            @if($transaction->tax_amount)
                                <h5 class="text-uppercase text-right"><strong>Taxes</strong>: $ {{ \App\Helpers\Helper::formatNumber($transaction->tax_amount)}}</h5>
                            @endif
                            <hr>
                            <h5 class="text-right"><strong>Total: $ {{ \App\Helpers\Helper::formatNumber($transaction->total) }}</strong></h5>
                        </div>
                    </td>
                </tr>
            </tbody>
