    ...
    <div class="transaction-print container">
        <div class="row">
            <div class="col-md-4 text-center">
                <img width="20%" src="{{ asset('admin/assets/img/vendor-logos/'. $transaction->vendor->logo .'.png') }}" alt="Logo">
        ...
        <div class="row">
            <div class="col-md-4 offset-md-8">
                <p class="text-right total-top">TOTAL: $ {{ number_format(($transaction->total)?$transaction->total:0, 2, '.', '') }}</p>
            </div>
        </div>
        <div class="row">
            ...
            <tbody>
                @foreach($transaction->purchase as $purchase)
                    <tr>
                        ...
                        <td style="border-right: 1px solid rgba(230, 230, 230, 0.7);">
                            <div style="display: flex; justify-content: flex-end; align-items: center;"> $ {{ number_format(($purchase->price)?$purchase->price:0, 2, '.', '') }}</div>
                        </td>
                    </tr>
                @endforeach
                <tr>
                    ...
                    <td colspan="2" style="border-right: 1px solid rgba(230, 230, 230, 0.7);">
                        <div>
                            <h5 class="text-uppercase text-right"><strong>Subtotal</strong>: $ {{ number_format(($transaction->sub_total)?$transaction->sub_total:0, 2, '.', '') }}</h5>
                            ...
                            @if($transaction->tax_amount)
                                <h5 class="text-uppercase text-right"><strong>Taxes</strong>: $ {{ number_format(($transaction->tax_amount)?$transaction->tax_amount:0, 2, '.', '')}}</h5>
                            @endif
                            <hr>
                            <h5 class="text-right"><strong>Total: $ {{ number_format(($transaction->total)?$transaction->total:0, 2, '.', '') }}</strong></h5>
                        </div>
                    </td>
                </tr>
            </tbody>
