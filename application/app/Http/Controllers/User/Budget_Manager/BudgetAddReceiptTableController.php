...
class BudgetAddReceiptTableController extends Controller
{
    ...
    {
        $transactions = Transaction::where('user_id', Auth::user()->id)->where('budget_id', '=', NULL)->where('is_hidden', '=', 0)->where('type', '=', 0);
        ...
        if (request()->from) {
            ...
            $transactions = $transactions->where('transaction_date', '>=', $from->format('Y-m-d h:m:s'));
        }
        if (request()->to) {
            $to=Carbon::parse(request()->to);
            $transactions = $transactions->where('transaction_date', '<=', $to->format('Y-m-d'));
        }
